...
def register_idea(request, program_id):
    ...
    return render(
        request, "ideas/register_idea.html", {"form": form, "program": program}
    )
...
def my_ideas(request):
    ...
    search_query = request.GET.get(
        "search", ""
    ).strip()  # Búsqueda por título o descripción
...
def edit_idea(request, idea_id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.success(
                request,
                "La idea fue actualizada exitosamente y está pendiente de revisión.",
            )
...
def committee_review_ideas(request):
    ...
    ideas = Idea.objects.filter(
        Q(state="reviewed_by_coordinator")
        | Q(state="committee_in_review")
        | Q(state="coordinator_rejected"),
    ...
    return render(
        request,
        "committe/committee_review_ideas.html",
        {
            "ideas": ideas,
...
def committee_review_idea(request, idea_id):
    ...
    return render(
        request,
        "committe/committee_review_idea.html",
        {
            "idea": idea,
...
def coordinator_assign_team_detail(request, idea_id):
    ...
    team_members = User.objects.filter(
        profile__roles__name="Empleado"
    )  # Filtrar empleados

    if request.method == "POST":
        team_name = request.POST.get("team_name")
        selected_members = request.POST.getlist(
            "team_members"
        )  # Obtener miembros seleccionados

        if not team_name or not selected_members:
            messages.error(
                request, "Debe asignar un nombre y al menos un miembro al equipo."
            )
        else:
    ...
    return render(
        request,
        "coordinator/assign_team_detail.html",
        {
            "idea": idea,
...
def sponsor_review_detail(request, idea_id):
    ...
    sponsor_decision = SponsorDecision.objects.filter(
        idea=idea, sponsor=request.user
    ).first()

    return render(
        request,
        "sponsor/review_detail.html",
        {
            "idea": idea,
...
def team_edit_idea(request, idea_id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.success(
                request, "La idea fue actualizada exitosamente y está en progreso."
            )
    ...
    return render(
        request,
        "coordinator/team_edit_idea.html",
        {
            "form": form,
            "program": program,
            "idea": idea,
