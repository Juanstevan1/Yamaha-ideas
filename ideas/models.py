...
class Program(models.Model):
    ...
    form_config = models.JSONField(blank=True, null=True)  # Define campos adicionales para ideas.
    committee_members = models.ManyToManyField(User, related_name="committee_programs", blank=True)
...
class Idea(models.Model):
    ...
    employee = models.ForeignKey(User, on_delete=models.CASCADE, related_name='proposed_ideas')
    program = models.ForeignKey(Program, on_delete=models.CASCADE, related_name='ideas', null=True, blank=True)
    ...
    assigned_team = models.ManyToManyField(User, related_name="team_assigned_ideas", blank=True)
    ...
    def calculate_committee_decision(self):
        ...
        approved_votes = self.ratings.filter(rating__gte=3).count()  # Asumiendo que una calificación >= 3 es una aprobación
...
class CommitteeRating(models.Model):
    committee_member = models.ForeignKey(User, on_delete=models.CASCADE, related_name="given_ratings")
...
class Team(models.Model):
    ...
    idea = models.OneToOneField(Idea, on_delete=models.CASCADE, related_name="team", null=True, blank=True)
